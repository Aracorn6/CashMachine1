...
public class BanknoteInfo {
    public static int[] readInfo() {
        int[] intBanknoteInfo = {};
        try (BufferedReader reader = new BufferedReader(new FileReader("BanknoteInfo.txt"))) {
            String[] splitBanknoteInfo = reader.readLine().split(" ");
    ...
    public static void writeInfo(String info) {
        try (FileWriter writer = new FileWriter("BanknoteInfo.txt")) {
            writer.write(info);
        } catch (IOException ex) {
            ex.printStackTrace();
        }
